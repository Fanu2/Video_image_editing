...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.download_mp3_btn.clicked.connect(self.download_mp3_from_url)
        self.load_url_btn.clicked.connect(self.load_video_from_url)
        ...
        top_layout.addWidget(self.download_mp3_btn)
    ...
    @Slot()
    def download_mp3_from_url(self):
        ...
        try:
            import yt_dlp
            outpath = QFileDialog.getSaveFileName(self, 'Save MP3 As', str(Path.home() / 'audio.mp3'), 'MP3 Files (*.mp3)')[0]
            if not outpath:
                return
            opts = {
            ...
            with yt_dlp.YoutubeDL(opts) as ydl:
                ydl.download([url])
            QMessageBox.information(self, 'MP3 Downloaded', f'Saved to: {outpath}')
        except Exception as e:
            QMessageBox.critical(self, 'MP3 Download Error', str(e))
