...
def create_movie(image_folder, output_video, display_duration=3, fps=24):
    ...
    # Repeat images to meet the display duration per image
    images_with_duration = []
    for image in images:
        images_with_duration.extend([image] * int(fps * display_duration))

    # Create a video clip from the numpy arrays
    video_clip = ImageSequenceClip(images_with_duration, fps=fps)
...
create_movie(image_folder, output_video, display_duration=1, fps=24)
