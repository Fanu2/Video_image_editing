...
def create_video_from_images(images, video_path, fps=24):
    """Creates a video from a list of images."""
    if not images:
        raise ValueError("No images to create video.")

    # Calculate the duration each image should be shown
    duration_per_image = 5 * 60 / len(images)  # 10 minutes total duration

    # Create a list of clips with the appropriate FPS
    clips = [ImageSequenceClip([img], durations=[duration_per_image]).set_duration(duration_per_image) for img in
             images]

    # Concatenate clips
    final_clip = concatenate_videoclips(clips, method='compose')
    final_clip.fps = fps  # Set FPS for the final video
    final_clip.write_videofile(video_path, codec='libx264')
    print(f"Video created successfully at: {video_path}")  # Debug print


def main():
# [REMOVED PATH]     odt_path = '/home/jasvir/Music/Movie work/Epub/Kolkata.odt'
# [REMOVED PATH]     output_folder = '/home/jasvir/Music/Movie work/Epub/frames/'
# [REMOVED PATH]     video_path = '/home/jasvir/Music/Movie work/Epub/Honeymoon1.mp4'
