...
def extract_text_from_odt(odt_path):
    """Extracts and returns text content from an ODT file."""
    doc = load(odt_path)
    text = ""
    ...
    for p in doc.getElementsByType(P):
        ...
        if item_text.strip():  # Check if there is non-empty text
            text += item_text + "\n\n"
...
def create_images_from_text(text, output_folder, img_width=1280, img_height=720, font_size=40, margin=40,
                            bg_color=(255, 255, 255), text_color=(0, 0, 0)):
    """Creates images from text and saves them in the output folder."""
    ...
    if not lines:
        raise ValueError("No lines created from text.")
...
# [REMOVED PATH]     video_path = '/home/jasvir/Music/Movie work/Epub/Collection_of_Poems.mp4'
