...
def create_animation(input_folder, output_video):
    ...
    # Calculate duration based on number of clips (each clip contributes 1 second)
    duration = len(clips) / 24  # Assuming 24 frames per second

    # Create a video clip from the images and set the duration
    video_clip = ImageSequenceClip(clips, fps=24).set_duration(duration)

    # Set the video clip to loop and set its duration
    looped_clip = video_clip.loop().set_duration(duration)

    # Write the video to a file
    looped_clip.write_videofile(output_video, codec='libx264', fps=24)
    print(f"Animation saved as {output_video}")

# Example usage
# [REMOVED PATH] input_folder = "/home/jasvir/Documents/Princess Jodha/Jodha9/"
# [REMOVED PATH] output_video = "/home/jasvir/Documents/Princess Jodha/Jodha9/video6.mp4"
